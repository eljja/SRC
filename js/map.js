...
class MapView {
  ...
  drawNodeMarker(node) {
    ...
    const size = Math.min(32, 16 + node.projects.length * 2);
    ...
    // Popup Content
    const projectListHtml = node.projects.slice(0, 3).map(p => `
    ...
    const popupHtml = `
      <div style="min-width: 220px; font-family: inherit;">
        ...
        <div style="font-size: 11px; color: #9ca3af; margin-bottom: 4px;">
          📍 ${node.city || ''}, ${node.country || ''} (${node.projects.length}건 연구 과제)
        </div>
        ${projectListHtml}
      </div>
    `;
  ...
  drawCurvedConnection(startLatLng, endLatLng, project) {
    ...
    const opacity = isActive ? 0.75 : 0.4;
    const weight = Math.min(5, Math.max(2, (project.funding_amount_usd || 1000000) / 10000000));
